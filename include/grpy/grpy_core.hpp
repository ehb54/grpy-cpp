...
//   * the compute path translated from Fortran to C++, on Eigen;
//   * the dense inverse replaced by a tiled, in-place Cholesky solved against the 11
//     right-hand sides the rigid-body reduction actually needs, so the full inverse is
//     never formed and only the upper triangle is stored;
//   * assembly and factorization parallelized over an injected thread pool;
//   * optional single-precision storage and factorization, halving the memory;
//   * optional out-of-core storage, bounding resident memory independently of model size;
//   * PI, KB and NA corrected to double precision (they were single, capping the whole
//     calculation at about seven digits).
//
...
inline T4 add( const T4&a,const T4&b,double s ){
   T4 t;
   for ( int i=0;i<3;i++ ) {
      for ( int j=0;j<3;j++ ) {
         for ( int k=0;k<3;k++ ) {
            for ( int l=0;l<3;l++ ) {
               t.d[ i ][ j ][ k ][ l ]=a.d[ i ][ j ][ k ][ l ]+s*b.d[ i ][ j ][ k ][ l ];
            }
         }
      }
   }
...
inline T4 scale( const T4&a,double s ){
   T4 t;
   for ( int i=0;i<3;i++ ) {
      for ( int j=0;j<3;j++ ) {
         for ( int k=0;k<3;k++ ) {
            for ( int l=0;l<3;l++ ) {
               t.d[ i ][ j ][ k ][ l ]=s*a.d[ i ][ j ][ k ][ l ];
            }
         }
      }
   }
...
inline Eigen::Matrix<double,5,5> ddY2( const T4& cart ){
   ...
   for ( int i=0;i<5;i++ ) {
      for ( int j=0;j<5;j++ ) {
         b( i,j )=mulT2aT4T2b( Y2()[ i ],cart,Y2()[ j ] );
      }
   }
   return b;
}
...
inline void assemble( const std::vector<Bead>& b, int N,
   ...
   for ( int i=0;i<N;i++ ) {
      ...
      for ( int j=0;j<5;j++ ) {
         AQQ( 5*i+j,5*i+j )=3.0/( 20.0*a*a*a );
      }
   }
...
inline void assemble_tiled( const std::vector<Bead>& b, int N, la::TiledUpperSPD<S>& M,
   ...
   for ( int i = 0; i < N; ++i ) {                               // self-terms (O(N), serial)
      ...
      for ( int j = 0; j < 5; ++j ) {
         M.at( Q+5*i+j, Q+5*i+j ) = ( S )( 3.0/( 20.0*a*a*a ) );
      }
   }
   ...
   for ( int base = 0; base < N; ) {
      ...
      for ( int t = 0; t < n; ++t ) {
         pairs_done += (long long) ( N - 1 - ( base + t ) );
      }
      base += n;
      if ( prog ) {
...
inline MatrixXd spd_inverse( const MatrixXd& Ain ) {
   ...
   for ( int i=0;i<n;i++ ) {  // mirror upper->lower
      for ( int j=0;j<i;j++ ) {
         inv( i,j )=inv( j,i );
      }
   }
...
inline double brown_ew( const Eigen::Matrix<double,11,11>& AR ){
   ...
   for ( int i=0;i<3;i++ )for ( int j=0;j<3;j++ )for ( int k=0;k<3;k++ )for ( int l=0;l<3;l++ ){
      ...
      for ( int p=0;p<3;p++ ) {
         for ( int q=0;q<3;q++ ) {
            ede+=EPS( i,k,p )*EPS( j,l,q )*MRR( p,q );
         }
      }
