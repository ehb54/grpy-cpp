...
inline void f100( std::ostream& o, const std::string& s, double v, const std::string& u ) {
   o << " " << aw( s,70 ) << es( v ) << aw( u,11 ) << "\n";
}
inline void f105( std::ostream& o, double a, double b, double c ) { o << " " << es( a ) << es( b ) << es( c ) << "\n"; }
inline void f106( std::ostream& o, const std::string& a, const std::string& b, const std::string& c ) {
   o << " " << aw( a,11 ) << aw( b,11 ) << aw( c,11 ) << "\n";
}
inline void f102( std::ostream& o, const std::string& a, const std::string& b ) { o << " " << aw( a,40 ) << aw( b,15 ) << "\n"; }
inline void f107( std::ostream& o, const std::string& s, double a, double b, double c ) {
   o << " " << aw( s,11 ) << es( a ) << es( b ) << es( c ) << "\n";
}
...
inline void write_report( std::ostream& o, const ReportData& d ) {
   ...
   f100( o, "Harmonic mean (correlation) time:",
        5.0 / ( 1.0/TAU[ 1 ]+1.0/TAU[ 2 ]+1.0/TAU[ 3 ]+1.0/TAU[ 4 ]+1.0/TAU[ 5 ] )
        * ( std::pow( U,3 )*PI*ETA )/( KB*TK ),
        "[s]" "       " );
   f100( o, "Sedimentation coefficient (Mw Dlt (1. - (vbar*rho))/(nA kB T)):",
        d.MW*( 1.0-( d.VBAR*d.RHO ) )*1.0e+13/NA
        * ( ( ATRCH[ 1 ][ 1 ]+ATRCH[ 2 ][ 2 ]+ATRCH[ 3 ][ 3 ] )/3.0 )/( U*PI*ETA ),
        "[Svedberg]" );
   blank( o );
   ...
   blank( o );
   f100( o, "Translational diffusion coefficient:",
         ( ( ATR[ 1 ][ 1 ]+ATR[ 2 ][ 2 ]+ATR[ 3 ][ 3 ] )/3.0 )*KB*TK/( U*PI*ETA ), "[cm^2/s]" "  " );
   blank( o ); f102( o, " 6x6 diffusion matrix: ", "Dtt  Dtr" ); blank( o ); f102( o, "", "Drt  Drr" ); blank( o );
   auto fillDTR = [ & ]( const double M[ 7 ][ 7 ] ) {
      for ( int i=1;i<=3;++i ) {
         for ( int j=1;j<=3;++j ) {
            DTR[ i ][ j ] = M[ i ][ j ]*KB*TK/( U*PI*ETA );
         }
      }
      for ( int i=4;i<=6;++i ) {
         for ( int j=4;j<=6;++j ) {
            DTR[ i ][ j ] = M[ i ][ j ]*KB*TK/( std::pow( U,3 )*PI*ETA );
         }
      }
      for ( int i=1;i<=3;++i ) {
         for ( int j=4;j<=6;++j ) {
            DTR[ i ][ j ] = M[ i ][ j ]*KB*TK/( std::pow( U,2 )*PI*ETA );
         }
      }
      for ( int i=4;i<=6;++i ) {
         for ( int j=1;j<=3;++j ) {
   ...
   auto dumpDTR = [ & ]() {
      for ( int i=1;i<=3;++i ) {
         f101( o, DTR[ i ][ 1 ],DTR[ i ][ 2 ],DTR[ i ][ 3 ],DTR[ i ][ 4 ],DTR[ i ][ 5 ],DTR[ i ][ 6 ] );
      }
      blank( o );
      for ( int i=4;i<=6;++i ) {
         f101( o, DTR[ i ][ 1 ],DTR[ i ][ 2 ],DTR[ i ][ 3 ],DTR[ i ][ 4 ],DTR[ i ][ 5 ],DTR[ i ][ 6 ] );
   ...
   blank( o );
   f100( o, "Translational diffusion coefficient:",
         ( ( ATRCH[ 1 ][ 1 ]+ATRCH[ 2 ][ 2 ]+ATRCH[ 3 ][ 3 ] )/3.0 )*KB*TK/( U*PI*ETA ), "[cm^2/s]" "  " );
   blank( o ); f102( o, " 6x6 diffusion matrix: ", "Dtt  Dtr" ); blank( o ); f102( o, "", "Drt  Drr" ); blank( o );
   fillDTR( ATRCH ); dumpDTR(); blank( o );
   f100L( o, "Radius of the sphere with equal:" ); blank( o );
   f100( o, "Translational diffusion coefficient:",
         U/( ( ATRCH[ 1 ][ 1 ]+ATRCH[ 2 ][ 2 ]+ATRCH[ 3 ][ 3 ] )/3.0 )/6.0, "[cm]" "      " );
