...
inline void derive_scalars( const ReportData& d, Results& r ) {
   ...
   for ( int i=0;i<3;++i ) {
      r.tau_vector[ i ] = d.TAU[ 6+i ]*tconv;
   }
   for ( int i=0;i<5;++i ) {
      r.tau_tensor[ i ] = d.TAU[ 1+i ]*tconv;
   }
   ...
   auto fill6 = [ & ]( const double M[ 7 ][ 7 ], double out[ 6 ][ 6 ] ) {
      for ( int i=1;i<=3;++i ) {
         for ( int j=1;j<=3;++j ) {
            out[ i-1 ][ j-1 ] = M[ i ][ j ]*KB*TK/( U*PI*ETA );
         }
      }
      for ( int i=4;i<=6;++i ) {
         for ( int j=4;j<=6;++j ) {
            out[ i-1 ][ j-1 ] = M[ i ][ j ]*KB*TK/( std::pow( U,3 )*PI*ETA );
         }
      }
      for ( int i=1;i<=3;++i ) {
         for ( int j=4;j<=6;++j ) {
            out[ i-1 ][ j-1 ] = M[ i ][ j ]*KB*TK/( std::pow( U,2 )*PI*ETA );
         }
      }
      for ( int i=4;i<=6;++i ) {
         for ( int j=1;j<=3;++j ) {
